...
print("1. Zip taranıyor ve 40-60 yaş grubu filtreleniyor...")

with zipfile.ZipFile(zip_path, 'r') as z:
    z_list = z.namelist()
    available_subjects = []
    for f in z_list:
        if f.endswith('.nii') and 'IXI' in f: # .nii.gz kontrolü
...
# 3. 40-60 YAŞ ARASI EN YÜKSEK HATALARI BULMA
res = df_final.loc[X_test.index].copy()
res['Tahmin'] = preds
res['Hata'] = abs(res['Tahmin'] - res['age'])
orta_yas_hatalilar = res[(res['age'] >= 40) & (res['age'] <= 60)]
worst_5_orta = orta_yas_hatalilar.sort_values('Hata', ascending=False).head(5)

# 4. GÖRSELLEŞTİRME VE PDF KAYDETME
if worst_5_orta.empty:
    print("HATA: Test setinde bu yaş aralığında hasta bulunamadı!")
else:
    print(f"2. {len(worst_5_orta)} vaka çiziliyor ve PDF kaydediliyor...")
    with zipfile.ZipFile(zip_path, 'r') as z:
        fig, axes = plt.subplots(5, 3, figsize=(16, 25))
        planes = ["Sagittal (X)", "Coronal (Y)", "Axial (Z)"]

        for i, (idx, row) in enumerate(worst_5_orta.iterrows()):
        ...
        plt.suptitle("40-60 Yaş Arası En Büyük Tahmin Hataları Analizi", fontsize=20, y=1.01)
        
        # --- KAYDETME KISMI BURADA OLMALI ---
        plt.tight_layout()
        plt.savefig("orta_yas_vaka_analizi.pdf", format='pdf', bbox_inches='tight', dpi=300)
        print("Grafik 'orta_yas_vaka_analizi.pdf' adıyla kaydedildi.")
        
        # En son ekranda göster
        plt.show()

    print("\n--- 40-60 YAŞ GRUBU EN HATALI 5 HASTA ---")
    print(worst_5_orta[['subject_id', 'age', 'Tahmin', 'Hata', 'sex', 'scanner', 'site']].to_string(index=False))